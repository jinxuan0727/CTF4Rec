...
def wasserstein_distance_matmul(mean1, cov1, mean2, cov2):
        ...
        cov_ret = -2 * torch.matmul(torch.sqrt(torch.clamp(cov1, min=1e-24)),
                                    torch.sqrt(torch.clamp(cov2, min=1e-24)).transpose(-1,
                                                                                       -2)) + cov1_2 + cov2_2.transpose(
            -1, -2)

        return ret + cov_ret


class FDFEmbedding(nn.Module):
    ...
    def __init__(self, field_dims, offsets, embed_dim):
        super(FDFEmbedding, self).__init__()
...
class GMA_MultiHeadAttention(nn.Module):
    """
    ...
    def __init__(self, n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps):
        super(GMA_MultiHeadAttention, self).__init__()
    ...
    def forward(self, input_tensor, input_mean_tensor, input_cov_tensor, attention_mask, is_f):
        ''''''
        ...
        _new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        new_context_layer_shape = mean_context_layer.size()[:-2] + (self.all_head_size,)

        context_layer = context_layer.view(*_new_context_layer_shape)
        ...
        return mean_hidden_states, cov_hidden_states, hidden_states
...
class GMA_TransformerLayer(nn.Module):
    """
    ...
    def __init__(
        ...
        super(GMA_TransformerLayer, self).__init__()
        self.multi_head_attention = GMA_MultiHeadAttention(
            n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps
        )
        self.feed_forward = FeedForward(hidden_size, intermediate_size, hidden_dropout_prob, hidden_act, layer_norm_eps)

    def forward(self, hidden_states, attention_mask, mean_sequence_emb, cov_sequence_emb, is_f):
        mean_attention_output, cov_attention_output, attention_output = self.multi_head_attention(hidden_states,
                                                                                                  mean_sequence_emb,
                                                                                                  cov_sequence_emb,
                                                                                                  attention_mask,
                                                                                                  is_f)
        ...
        # feedforward_output = self.feed_forward(attention_output)
            feedforward_output = self.feed_forward(mean_attention_output)
        else:
            feedforward_output = self.feed_forward(attention_output)
...
class GMA_TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        super(GMA_TransformerEncoder, self).__init__()
        layer = GMA_TransformerLayer(
            n_heads, hidden_size, inner_size, hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps
        )
        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(n_layers)])

    def forward(self, hidden_states, attention_mask, mean_sequence_emb, cov_sequence_emb, is_f, output_all_encoded_layers=True):
    # def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attention_mask, mean_sequence_emb, cov_sequence_emb, is_f)
...
class FDFEncoder(nn.Module):
    ...
    def __init__(
        ...
        super(FDFEncoder, self).__init__()
        self.n_layers = n_layers
        self.layer = nn.ModuleList()
        for n in range(self.n_layers):
            self.layer_ramp = FDFBlock(n_heads, hidden_size, inner_size, hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps, n, config)
...
class FMFirstOrderLinear(nn.Module):
    ...
    def __init__(self, config, dataset, output_dim=1):
        ...
        if len(self.token_field_dims) > 0:
            ...
            self.token_embedding_table = FDFEmbedding(self.token_field_dims, self.token_field_offsets, output_dim)
